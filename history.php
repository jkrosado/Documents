...
</li></ul></div></div></div><div id="results" style="padding-top: 10px;">
...
<?php
                    $stmt = $conn->prepare("SELECT * FROM views WHERE viewer = ? ORDER BY viewed DESC");
                    $stmt->bind_param("s", $_SESSION['siteusername']);
                    $stmt->execute();
                    $result = $stmt->get_result();
                    if($result->num_rows==0) {
                        echo "<p style='padding-left: 10px; padding-top: 10px;'>Your history is empty.</p>";
                    }
                    while($hist = $result->fetch_assoc()) { 
                ?>
<?php 
$videos = $_video_fetch_utils->fetch_video_rid($hist['videoid']);
echo "<p style='margin-left: 15px;'>Viewed on <b>" . date("F j, Y", strtotime($hist['viewed'])) . "</b></p>";
if($videos == 0) {
    $video = $_video_fetch_utils->fetch_video_rid("UKY3scPIMd8");
} else {
    $video = $_video_fetch_utils->fetch_video_rid($hist['videoid']);
}
$publishedReadable6 = date( "F j, Y", strtotime($video['publish']) ); 
?>
