...
<div id="guide">    <div id="guide-container">
    ...
    <a class="guide-item narrow-item" href="/2013" title="Popular on FulpTube" data-channel-id="youtube">
...
<?php } else { ?>
<?php
                    $stmt = $conn->prepare("SELECT * FROM `users` WHERE partner = 'y' ORDER BY id LIMIT 15");
                    $stmt->execute();
                    $result = $stmt->get_result();
                    while($sub = $result->fetch_assoc()) { 
                ?>

<a class="guide-item " href="/2013/profile?id=<?php echo $sub['username']; ?>" title="<?php echo htmlspecialchars($sub['username']); ?>" data-channel-id="UCBR8-60-B28hp2BmDPdntcQ">
        <span class="thumb"><span class="video-thumb ux-thumb yt-thumb-square-18 "><span class="yt-thumb-clip"><span class="yt-thumb-clip-inner"><img data-thumb="/dynamic/pfp/<?php echo $sub['pfp']; ?>" alt="Thumbnail" data-thumb-manual="1" src="/dynamic/pfp/<?php echo $sub['pfp']; ?>" width="18"><span class="vertical-align"></span></span></span></span></span>
      <span class="display-name">
